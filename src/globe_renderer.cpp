...
GlobeRenderer::GlobeRenderer(int cx, int cy, int radius)
    : _cx(cx), _cy(cy), _radius(radius),
      _tiltDeg(20.0f), _currentLonDeg(-88.0f), _targetLonDeg(-88.0f),
      _autoSpin(true), _spinSpeed(8.5f), _pulsePhase(0.0f),
      _lut(nullptr), _lutCount(0) {}
...
void GlobeRenderer::precomputeLUT() {
    ...
    _lut = (SpherePixel*)heap_caps_malloc(maxPixels * sizeof(SpherePixel), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!_lut) {
    ...
    for (int dy = -R; dy <= R; dy++) {
        for (int dx = -R; dx <= R; dx++) {
            ...
            if (r2 <= R2) {
                ...
                // Texture coordinates
                int v = (int)(((float)M_PI * 0.5f - lat) / (float)M_PI * (float)EARTH_TEX_HEIGHT);
                v = std::max(0, std::min(EARTH_TEX_HEIGHT - 1, v));

                int uBase = (int)((lonRel + (float)M_PI) / (2.0f * (float)M_PI) * (float)EARTH_TEX_WIDTH);
...
void GlobeRenderer::update(float dt) {
    ...
    if (_autoSpin) {
        ...
        if (_currentLonDeg < -180.0f) _currentLonDeg += 360.0f;
        _targetLonDeg = _currentLonDeg;
    } else {
        // Smoothly interpolate towards target longitude
        float diff = _targetLonDeg - _currentLonDeg;
        while (diff > 180.0f) diff -= 360.0f;
        while (diff < -180.0f) diff += 360.0f;
        _currentLonDeg += diff * (dt * 5.0f);
    }
}

void GlobeRenderer::setRotation(float lonDeg) {
    while (lonDeg > 180.0f) lonDeg -= 360.0f;
    while (lonDeg < -180.0f) lonDeg += 360.0f;
...
void GlobeRenderer::spinBy(float deltaDeg) {
    ...
    while (_targetLonDeg > 180.0f) _targetLonDeg -= 360.0f;
    while (_targetLonDeg < -180.0f) _targetLonDeg += 360.0f;
...
void GlobeRenderer::centerOnRoute(const FlightRoute& route) {
    // Calculate midpoint longitude of route
    float midLon = (route.originCoords.lon + route.destCoords.lon) * 0.5f;
    _targetLonDeg = midLon;
    _autoSpin = false;
}
...
bool GlobeRenderer::projectPoint(const Vec3& pt, float lonDeg, float tiltDeg, int16_t& outX, int16_t& outY, float& outZ) {
...
void GlobeRenderer::render(LGFX_Sprite& target, int activeRouteIndex) {
    ...
    int uOffset = (int)std::round(_currentLonDeg * ((float)EARTH_TEX_WIDTH / 360.0f));
    ...
    for (size_t i = 0; i < _lutCount; i++) {
        ...
        uint16_t texColor = pgm_read_word(&EARTH_TEXTURE[sp.v * EARTH_TEX_WIDTH + u]);
    ...
    for (size_t rIdx = 0; rIdx < routes.size(); rIdx++) {
        ...
        uint16_t arcColor = isActive ? target.color565(255, 220, 60) : target.color565(255, 140, 20);

        int16_t prevX = 0, prevY = 0;
        float prevZ = -1.0f;
        bool hasPrev = false;

        for (const auto& pt : r.arcPoints) {
            int16_t sx, sy;
            float sz;
        ...
        if (isActive && !r.arcPoints.empty()) {
            float t = _pulsePhase;
            size_t segCount = r.arcPoints.size() - 1;
            float segF = t * (float)segCount;
            size_t idx0 = (size_t)segF;
            size_t idx1 = std::min(idx0 + 1, segCount);
            float frac = segF - (float)idx0;

            const Vec3& p0 = r.arcPoints[idx0];
            const Vec3& p1 = r.arcPoints[idx1];
            Vec3 pulsePt(
                p0.x + (p1.x - p0.x) * frac,
                p0.y + (p1.y - p0.y) * frac,
                p0.z + (p1.z - p0.z) * frac
            ...
            int16_t px, py;
            float pz;
            if (projectPoint(pulsePt, _currentLonDeg, _tiltDeg, px, py, pz) && pz > 0.05f) {
    ...
    for (const auto& codeStr : uniqueAirports) {
        LatLon coords;
        std::string name;
        if (!flightData.findAirport(codeStr.c_str(), coords, name)) continue;

        Vec3 unitPt = latLonToVec3(coords.lat, coords.lon, 1.0f);
        int16_t sx, sy;
        float sz;
        if (projectPoint(unitPt, _currentLonDeg, _tiltDeg, sx, sy, sz) && sz > 0.08f) {
            bool isActiveAirport = false;
            if (activeRouteIndex >= 0 && activeRouteIndex < (int)routes.size()) {
                const auto& cur = routes[activeRouteIndex];
                if (strcmp(cur.origin, codeStr.c_str()) == 0 || strcmp(cur.dest, codeStr.c_str()) == 0) {
    ...
    for (size_t i = 0; i < drawnAirports.size(); i++) {
        ...
        for (size_t j = 0; j < drawnAirports.size(); j++) {
            ...
            if (std::abs(dx) < 18 && std::abs(dy) < 16) {
                // If this is CMI and ORD is above, push CMI down
        ...
        if (labelX > SCREEN_WIDTH - 25) labelX = a.x - 24;
        if (labelY < 4) labelY = 4;
        if (labelY > SCREEN_HEIGHT - 12) labelY = SCREEN_HEIGHT - 12;
        ...
        target.setTextColor(a.active ? target.color565(255, 230, 80) : target.color565(240, 245, 255));
        target.drawString(a.code, labelX, labelY);
    }

    // 5. Active Route Info Banner (bottom of 480x480 round screen)
    if (activeRouteIndex >= 0 && activeRouteIndex < (int)routes.size()) {
        const auto& cur = routes[activeRouteIndex];
        char banner[64];
        snprintf(banner, sizeof(banner), "%s  ->  %s   %.0f NM", cur.origin, cur.dest, cur.distanceNM);
