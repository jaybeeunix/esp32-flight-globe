...
FlightDataManager::FlightDataManager()
    : _sdMounted(false), _isDemoMode(false), _totalDistanceNM(0.0f),
...
bool FlightDataManager::initSD() {
  ...
  if (!SD_MMC.begin("/sdcard", true, false, 20000)) {
    ...
    if (!SD_MMC.begin("/sdcard", true, false, 10000)) {
      Serial.println("[SD] Warning: Failed to mount SD card. Check card "
                     "insertion and FAT32 format.");
      return false;
    }
  }
...
bool FlightDataManager::findAirport(const char *code, LatLon &outCoords,
                                    std::string &outName) {
...
bool FlightDataManager::parseLineToHops(const String &rawLine,
                                        std::vector<std::string> &outHops) {
  outHops.clear();
  String line = rawLine;
  line.trim();
  ...
  while (start < len) {
    ...
    if (token.length() == 3 && isAlpha(token[0]) && isAlpha(token[1]) &&
        isAlpha(token[2])) {
...
bool FlightDataManager::loadFlights(const char *filename) {
  ...
  if (!file)
    file = SD_MMC.open("/flights.txt", "r");
  if (!file)
    file = SD_MMC.open("/FLIGHTS.TXT", "r");
  if (!file)
    file = SD_MMC.open("/Flights.txt", "r");
  if (!file)
    file = SD_MMC.open("/flights.csv", "r");
  if (!file)
    file = SD_MMC.open("/FLIGHTS.CSV", "r");
  ...
  while (file.available()) {
    ...
    for (size_t i = 0; i + 1 < hops.size(); i++) {
      ...
      leg.distanceNM = calcDistanceNM(origCoords.lat, origCoords.lon,
                                      destCoords.lat, destCoords.lon);
      leg.distanceKM = calcDistanceKM(origCoords.lat, origCoords.lon,
                                      destCoords.lat, destCoords.lon);
    ...
    if (allLegsOk && !r.legs.empty()) {
      ...
      Serial.printf("[SD] Added route #%d: %s (%d legs, %.0f NM)\n", loaded,
                    r.itinerary.c_str(), (int)r.legs.size(), r.totalDistanceNM);
    }
  }
  file.close();
...
void FlightDataManager::loadDemoFlights() {
  ...
  const char *demoItineraries[] = {
      "BMI-DFW-HKG-DFW-BMI", "ORD-SFO-HND-LHR-ORD", "ORD-MIA-LIM-CUZ-LIM-ORD",
      "ORD-ZRH-NCE-ZRH-ORD", "ORD-DOH-KTM-DOH-ORD", "ORD-PHX-HNL-KOA-HNL-ORD",
      "LAX-SYD-CNS-SYD-LAX"};

  for (size_t i = 0; i < sizeof(demoItineraries) / sizeof(demoItineraries[0]);
       i++) {
    std::vector<std::string> hops;
    if (!parseLineToHops(String(demoItineraries[i]), hops))
      continue;
    ...
    for (size_t j = 0; j + 1 < hops.size(); j++) {
      ...
      if (findAirport(hops[j].c_str(), origCoords, origName) &&
        ...
        leg.distanceKM = calcDistanceKM(origCoords.lat, origCoords.lon,
                                        destCoords.lat, destCoords.lon);
        leg.arcPoints =
            generateGreatCircleArc(origCoords, destCoords, 32, 0.06f);
